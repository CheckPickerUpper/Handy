...
impl Device {
    fn build_stream_inner(
        ...
        let stream_creation_instant = std::time::Instant::now();
...
struct StreamInner {
    ...
    // ALSA sound-server bridges can return a zero callback timestamp even when
    // setup returned a non-zero value. This clock is the safe fallback for that
    // inconsistent bridge state.
    stream_creation_instant: std::time::Instant,
}
...
fn process_input(
    ...
    let callback = stream_timestamp(
        &status,
        stream.creation_instant,
        stream.stream_creation_instant,
    )?;
...
fn stream_timestamp(
    ...
    match creation_instant {
        None => {
            ...
            if nanos < 0 {
                return stream_timestamp_fallback(stream_creation_instant);
            }
